...
from GeoMIP.src.Method2_Dynamic_Programming_Reformulation.src.controllers.strategies.geometric import GeometricSIA
...
def analizador_geometrico():
    ...
    estrategia = GeometricSIA(gestor_mock)
...
def test_integridad_tabla_t(analizador_geometrico):
    ...
    costo_final = analizador_geometrico.tabla_transiciones[(key_inicio, key_fin)]
    assert not np.isnan(costo_final).any()
    assert np.all(costo_final >= 0.0)

@pytest.fixture
def estrategia_validada():
    """Configura un GeometricSIA controlado con dimensiones alineadas (N=3, 2^N=8)."""
    gestor_mock = MagicMock()
